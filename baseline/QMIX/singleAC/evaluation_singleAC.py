...
#     vaso_val = data.ori_vasopressors[data.ori_vasopressors!=0].quantile([0.25,0.5,0.75]).values
    ...
    data['ai_iv_quantile'] = data['ai_action_IV'].apply(lambda x: 1 if x<=iv_val[0] 
                                                                   else 2 if x<=iv_val[1] else 3 if x<=iv_val[2] else 4)
    
    data['ai_vasso_quantile'] = data['ai_action_Vasso'].apply(lambda x: 1 if x<=iv_val[0] 
                                                                         else 2 if x<=iv_val[1] else 3 if x<=iv_val[2] else 4)
    
    
    def compute_overall_ai_action(ai_iv_quantile, ai_vasso_quantile):
        overall_action = ai_iv_quantile*5 + ai_vasso_quantile  
        return overall_action



    data['ai_overal_continous'] = data.apply(lambda x: compute_overall_ai_action(x['ai_iv_quantile'], 
                                                                                x['ai_vasso_quantile']), axis=1)
    ...
    res_dir = 'result/'+'singleAC'+ '/'+ datetime.now().strftime('%Y-%m-%d-%H-%M-%S') + '_' + setting.REWARD_FUN + '_' + datatype +'_' + 'Itr'+str(setting.ITERATION_ROUND) + '_seed' + str(setting.SEED)+'_'+setting.MODEL + '/'
    if os.path.isdir(res_dir) == False:
        os.makedirs(res_dir)            
    data.to_csv(res_dir + 'check_cross.csv', encoding = 'gb18030')
